...
//        val reader =  new BlobFileReader("target/SeqFilWriterTest.dat")
...
//          
//          Assert.assertTrue(data.isDefined)
//          Assert.assertEquals(getKey(i), data.get.getKey)
...
//        val blobReader = new BlobFileReader("target/IdxReaderTest.dat")
//
//        
...
//          val data = blobReader.get(idx.get.getKey, idx.get.getPosition)
//          
//          Assert.assertTrue(data.isDefined)
...
//   }
    " get data to given index entry " in {
        ...
        val numDate = 20 // Number of test data
        
        
        val bigArray = new Array[Byte](50000)
        ...
        val writer = new SequenceFileWriter(s"hdfs://10.11.22.41:8020/bdq-blob/testblobs-${System.currentTimeMillis()}")
       
        for(i <- 0 to numDate) {
          println(getKey(i))
          writer.insert(getKey(i), i, bigArray)
        }
        writer.close
    }
   }
