...
if [ $LOCAL_MODE = true  ]; then
  ...
  exec $SPARK_SUBMIT --master $SPARK_MASTER --total-executor-cores $CORES --files $BASEDIR/conf/log4j.properties,$BASEDIR/conf/job-parameter.json --class org.scray.examples.ingest.nrw.NrwTrafficDataClient target/nrw-traffic-data-client-1.0-SNAPSHOT-jar-with-dependencies.jar ${ARGUMENTS[@]}
else
  exec $SPARK_SUBMIT --master $SPARK_MASTER --total-executor-cores $CORES --files $BASEDIR/conf/log4j.properties,$BASEDIR/conf/job-parameter.json --class org.scray.examples.ingest.nrw.NrwTrafficDataClient target/nrw-traffic-data-client-1.0-SNAPSHOT-jar-with-dependencies.jar ${ARGUMENTS[@]}
fi
