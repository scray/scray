...
object MetricsCrawler {
  ...
  def main(args: Array[String]) {
    ...
    val maxStepSize = 30 * 60 // Max timespan for a prometheus query
    ...
    for (i <- 0 to Int.MaxValue) {
      ...
      if (currentApiRequest != 0) {
        while (Math.abs(nextApiRequest - currentApiRequest) > maxStepSize) {

          logger.info(s"Requested range from ${new Date(currentApiRequest * 1000)} to ${new Date(nextApiRequest * 1000)} exeeds max step size. Query will be splited in chunks of ${maxStepSize} seconds")

          val timerangeEnd = currentApiRequest + maxStepSize
          queryMetrics(prometheusMetriklistUrl, currentApiRequest, timerangeEnd, metricsList, destinationPath + currentApiRequest + "_")
          currentApiRequest = timerangeEnd
        }
      }
  ...
  def queryMetrics(prometheusMetriklistUrl: String, dataStartDate: Long, dataEndDate: Long, matricsToQuery: List[String], outPath: String) = {
    ...
    val numMetricsWritte = matricsToQuery.foldLeft(0)((acc, metric) => {
      ...
      logger.debug("Query metric: " + nextMetricQuery)
