...
class YahooStreamAdapter extends LazyLogging {
  ...
  def getStream2 = {
    ...
    while(true) {
      ...
      parser.parseAll(parser.start, receivedData).map { x => println(x) }
    }
      ...
      class YahooStockStreamParser extends RegexParsers {
        ...
        def shareObject = OBJECT_PREFIX ~> STRING ~ ":{" ~
          ...
          h00 ~
          _
          => {new Share(name,  toFloat(l10) , toFloat(a00), toFloat(b00), toFloat(g00), toFloat(h00))}
        }
