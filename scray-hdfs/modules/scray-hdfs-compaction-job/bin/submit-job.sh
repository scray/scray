...
if [ $LOCAL_MODE = true  ]; then
  ...
  exec $SPARK_SUBMIT --master $SPARK_MASTER --total-executor-cores $CORES --files $BASEDIR/../conf/log4j.properties,$BASEDIR/../conf/job-parameter.json --class scray.hdfs.compaction.ScrayHdfsCompactionJob target/compaction-job-1.0-SNAPSHOT-jar-with-dependencies.jar ${ARGUMENTS[@]}
else
  exec $SPARK_SUBMIT --master $SPARK_MASTER --total-executor-cores $CORES --files $BASEDIR/../conf/log4j.properties,$BASEDIR/../conf/job-parameter.json --class scray.hdfs.compaction.ScrayHdfsCompactionJob target/compaction-job-1.0-SNAPSHOT-jar-with-dependencies.jar ${ARGUMENTS[@]}
fi
cd $ORIGDIR
