...
//
//class SequenceFileWriterTextBytesSpecs extends WordSpec with LazyLogging {
//
...
//    " read idx" in {
//
//      val writer = new BinarySequenceFileWriter("target/SeqFilWriterTest", new OutputTextBytesWritable)
//
//      for (i <- 0 to 1000) {
...
//
//      val writer = new BinarySequenceFileWriter("target/IdxReaderTest", new OutputTextBytesWritable)
//
//      for (i <- 0 to numDate) {
...
//      val numDate = 1000 // Number of test data
//
//      val writer = new BinarySequenceFileWriter("target/IdxReaderTest1", new OutputTextBytesWritable)
//
//      for (i <- 0 to numDate) {
//        writer.insert(getKey(i), i, getValue(i).getBytes)
//      }
...
//      }
//     
//      val writer = new BinarySequenceFileWriter("target/IoStreamRWTest", new OutputTextBytesWritable)
