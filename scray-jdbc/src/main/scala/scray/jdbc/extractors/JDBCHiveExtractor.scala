 ...
 */
class JDBCHiveExtractors[Q <: DomainQuery, S <: JDBCHiveQueryableSource[Q]](
    ti: TableIdentifier, hikari: HikariDataSource, metadataConnection: Connection,
    sqlDialect: ScraySQLDialect, futurePool: FuturePool) extends StoreExtractor[JDBCHiveQueryableSource[Q]] with LazyLogging {

  private val queryMapping = new DomainToHiveSQLQueryMapping[Q, S]
  
  // cache metadata connection for later use
  private val dbmsMetadata  = JDBCHiveExtractors.fetchOrGetDatabaseMetadata(ti.dbSystem, ti.dbId, metadataConnection)
  ...
  @tailrec private def scanColumns(resSet: ResultSet, columns: ListBuffer[JDBCColumn], typ: Boolean = true): Seq[JDBCColumn] = {
    logger.error("scanColumns:" + columns.size)
    
    if(resSet.next()) {
      logger.error("scanColumns next")
  ...
  val indexInformations = JDBCHiveExtractors.fetchOrGetIndexInformation(dbmsMetadata, ti)
  ...
  def getTableConfiguration(rowMapper: (_) => Row): TableConfiguration[_ <: DomainQuery, _ <: DomainQuery, _] = {
    ...
    val jdbcSource = Some(new JDBCHiveQueryableSource(ti,
        getRowKeyColumns, 
   ...
   */
  override def getTableIdentifier(store: JDBCHiveQueryableSource[Q], tableName: Option[String], dbSystem: Option[String]): TableIdentifier = 
   ...
   */
  override def createManualIndexConfiguration(column: Column,queryspaceName: String,version: Int,store: JDBCHiveQueryableSource[Q], 
 ...
 */
object JDBCHiveExtractors extends LazyLogging {
  ...
  def fetchOrGetDatabaseMetadata(dbms: String, dbId: String, connection: Connection): DatabaseMetaData = {
    lock.lock()
    try {
      logger.error(dbms + "|" + dbId)
      
      dbmsHash.get((dbms, dbId)).getOrElse { 
        val meta = connection.getMetaData
        
        logger.error("getMetaData" + meta.toString())
  ...
  def fetchOrGetIndexInformation(dbMeta: DatabaseMetaData, ti: TableIdentifier): Map[String, TemporaryIndexInfo] = {
    def getIndexInfos(rs: ResultSet, indexes: ListBuffer[JDBCHiveExtractors.TemporaryIndexInfo], 
        lastIndexRow: Option[JDBCHiveExtractors.TemporaryIndexInfo]): List[JDBCHiveExtractors.TemporaryIndexInfo] = {
      if(rs.next) {
        Option(rs.getString("INDEX_NAME")).map { _ =>
          val newIndex = JDBCHiveExtractors.TemporaryIndexInfo(rs.getString("COLUMN_NAME"), 
    ...
    try {
      indexHash.get(ti).getOrElse {
        ...
        try {
          ...
          val ee = new ListBuffer[JDBCHiveExtractors.TemporaryIndexInfo]
          val iInfo = getIndexInfos(rs, ee, None)
