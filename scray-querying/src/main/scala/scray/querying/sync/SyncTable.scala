...
//import scray.querying.sync.JobInitFunction


abstract case class Table[T <: Columns[_ <: Column]](val keySpace: String, val tableName: String, val columns: T) {}

trait Column {
  type T
  val name: String
  val dbTypeDetector: Column => String
  def getDBType = { dbTypeDetector(this) }
}

abstract class ColumnWithValue[ValueT] extends Column {
  val value: ValueT
}

class TypedColumn[U](nameV: String, dbTypeDetectorV: Column => String) extends Column {
  val name = nameV
  val dbTypeDetector = dbTypeDetectorV
  type T = U
}

class TypedColumnWithValue[U, ValueT](nameV: String, dbTypeDetectorV: Column => String, valueV: ValueT) extends ColumnWithValue {
  val name = nameV
  val dbTypeDetector = dbTypeDetectorV
  val value = valueV
  type T = U
...
abstract case class Columns[ColumnT <: Column]() { 
  val time: ColumnT
